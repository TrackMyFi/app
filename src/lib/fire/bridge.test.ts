...
describe('bridgeStatus', () => {
  const split = { accessible: 200_000, deferred: 400_000, ladderable: 400_000 }
  ...
  it('an underwater accessible balance projects to zero, not negative', () => {
    const r = bridgeStatus({ accessible: -10_000, deferred: 100_000, ladderable: 0 }, 40, 5, 40_000, 0.07, 0.03)
...
describe('bridgeStatus ladder', () => {
  ...
  it('grows the ladder bridge when pre-tax funds cannot fund every conversion year', () => {
    ...
    const split = { accessible: 200_000, deferred: 100_000, ladderable: 100_000 }
  ...
  it('reports no ladder when nothing is convertible', () => {
    const r = bridgeStatus({ accessible: 200_000, deferred: 400_000, ladderable: 0 }, 40, 5, 40_000, 0.07, 0.03)
  ...
  it('reports no ladder when the bridge fits inside the seasoning window', () => {
    ...
    const r = bridgeStatus({ accessible: 200_000, deferred: 400_000, ladderable: 400_000 }, 50, 6, 40_000, 0.07, 0.03)
    expect(r.needed).toBe(true)
    expect(r.ladder).toBeNull()
  })

  it('reports no ladder when no bridge is needed', () => {
    const r = bridgeStatus({ accessible: 200_000, deferred: 400_000, ladderable: 400_000 }, 50, 10, 40_000, 0.07, 0.03)
    expect(r.needed).toBe(false)
    expect(r.ladder).toBeNull()
  })
})
...
describe('bridgeContributionSplit', () => {
  ...
  it('buckets trailing-12-month contributions by account type', () => {
    const c = bridgeContributionSplit(accounts, [
      ...
      txn({ accountId: 3, amount: 600, date: '2026-02-01' }), // roth: neither bucket
      txn({ accountId: 4, amount: 1_000, date: '2026-01-01' }), // mixed: 60% traditional
    ], asOf)
    expect(c.accessible).toBeCloseTo(1_200 / 12, 6)
    expect(c.ladderable).toBeCloseTo((2_400 + 600) / 12, 6)
  })
...
describe('bridgeStatus with contributions', () => {
  ...
  it('adds the future value of ongoing contributions to both projections', () => {
    ...
    const r = bridgeStatus(split, 40, 5, 40_000, 0.07, 0.03, { accessible: 1_000, ladderable: 2_000 })
  ...
  it('taxable contributions alone can turn a thin bridge into a covered one', () => {
    const thin = { accessible: 50_000, deferred: 400_000, ladderable: 400_000 }
    const without = bridgeStatus(thin, 40, 5, 40_000, 0.07, 0.03)
    const withContrib = bridgeStatus(thin, 40, 5, 40_000, 0.07, 0.03, { accessible: 2_500, ladderable: 0 })
  ...
  it('a ladder can appear on contributions alone (zero pre-tax balance today)', () => {
    const noPreTax = { accessible: 100_000, deferred: 0, ladderable: 0 }
    const without = bridgeStatus(noPreTax, 40, 5, 40_000, 0.07, 0.03)
    const withContrib = bridgeStatus(noPreTax, 40, 5, 40_000, 0.07, 0.03, { accessible: 0, ladderable: 1_500 })
  ...
  it('net-withdrawal flows cannot push projections below zero', () => {
    const r = bridgeStatus({ accessible: 1_000, deferred: 0, ladderable: 0 }, 40, 5, 40_000, 0.07, 0.03, { accessible: -500, ladderable: 0 })
    expect(r.projectedAccessibleAtFi).toBe(0)
  })
})
